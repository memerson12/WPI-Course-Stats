...
def get_past_reports(course_id: str, year='2020-2021'):
...
def get_rating(report):
    return _get_rating(report[3], report[0], report[5])


def _get_rating(course_id, pidm_id, term):
    try:
        course_id_letters = re.search('[a-zA-Z]+', course_id).group()
        course_id_numbers = re.search('\d+', course_id).group()
        ...
        return _extract_report(raw_rating.split(';'))
    except Exception as e:
        print(course_id, "Error", e)
        return None


def _extract_report(fields):
    report_data = {
        "courseInfo": {
            "instructor": "",
            "courseTitle": "",
    ...
    all_entries = _process_done(fields[7])

    n0 = -1
    for j in range(19):
        n1 = _nth_index(all_entries, ",", 9, n0)
        token = all_entries[n0 + 1: n1]
        lots = token.split(',')
        if j < 7:
            keys = [*report_data['overallRatings']]
            _lots_to_object(report_data, lots, 'overallRatings', keys[j])
        elif j < 11:
            keys = [*report_data['relativeRating']]
            _lots_to_object(report_data, lots, 'relativeRating', keys[j - 7])
        elif j < 17:
            keys = [*report_data['frequentlyTrue']]
            _lots_to_object(report_data, lots, 'frequentlyTrue', keys[j - 11])
        elif j == 17:
            _lots_to_object(report_data, lots, 'misc', 'expectedGrade')
        else:
...
def _lots_to_object(report_data, lots, section, key):
...
def _nth_index(string: str, pat, n, i):
    L = len(string)
    while n != 0 and i < L:
        n -= 1
        i += 1
